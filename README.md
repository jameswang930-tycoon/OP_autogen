...
- 2026-08-03：文档去重——`HANDOFF_GLM52.md` 内容已被 PROGRESS（GLM52/V2 节）+ 新 `V2_ENV_ADAPTATION.md` 共同覆盖，删前把仅存于该文档的 3 处踩坑迁入 PROGRESS「GLM52 关键决定 #9」（module 对齐 EXTENSION_NAMESPACE、签名表不带模块、联调 triage 要点）后删除；新增 `V2_ENV_ADAPTATION.md`（V2 环境侧适配交接 A-F 清单）。pytest 224 passed
- 2026-08-03：V2 backend 重构（配置驱动 + 可扩展，开源最后一版）——`NgaBackend`/`AgentBackend` 合并为一个配置驱动 `NgaBackend`（通用 options→CLI 映射 + `extra_args` 透传 + 可选结构化输出路径/自由文本降级，**不预设 agent 能力**，未配退化为最基础调用）；删双模式/`GEN_PROMPT_MODE`，gen prompt 统一场景提示 + ext-* skill lazy-load（唯一路径）；`V2_ENV_ADAPTATION.md` §C 加 C.0 探测步骤（环境侧必做）。pytest 222 passed，冻结接缝零改动
- 2026-08-03：V2 预埋能力接口（优化知识 + 单元上限/利用率感知）——新增 3 个 `opt-*` optimization skill（按瓶颈一一对应）+ gen prompt `OPTIMIZATION_HINT` 段（按瓶颈提示触发，空段降级）；memory 加可选 `opt_technique_ref`/`utilization`（关联优化技巧、不存手册内容）；`Event` 加可选 `unit_peak` + `feedback_adapter.classify` 利用率分支（占比大且未饱和=真瓶颈、已饱和=约束，无数据/无阈值降级纯占比，阈值 `SATURATION_THRESHOLD` env 可配）。**所有预留项缺失降级为当前行为**；既有契约字段语义不变（仅新增可选字段/分支）。pytest 239 passed
- 2026-08-03：CLI backend 环境变量统一 `AGENT_*`、去硬编码 nga——`NGA_GENERATE_MODEL`/`NGA_CHOOSE_LEVER_MODEL`→`AGENT_*`，默认命令前缀 `['nga','run']`→`['agent','run']`；`preflight` 的 nga 二进制检查→agent（按 `AGENT_CMD` 首 token）；测试/文档同步。pytest 239 passed