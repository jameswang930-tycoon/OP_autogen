...
class PlannerAgent:
    ...
    def generate_v4(
        ...
        system_prompt = (
            ...
            f"2. 读优化策略文档: {playbook_path}  (★先读最前面的『速查卡』定位情况, 细节看『情况A~J』段; "
            f"全文按需读, 不用背; ★**结构层判断/离层 promote 前必读文末『结构层优化执行教学』** "
            f"(教学1/2/3 + 8 项检查清单) — 逐项核对当前 kernel 是否已做结构层优化: 有矩阵运算必须有 tl.dot, "
            f"因果 attention 必须有 kv_hi 剪枝, 归约已单遍; 缺任何一项 → 本轮先做结构层, 不许 promote)\n"
            ...
            f"5b. ★官方实测优先 (context 里 deep 侧): traffic_kb(实际搬运量)/bw_usage_rate(官方通路利用率)/active_bw(真实带宽) "
            f"优先于任何估算值; Tier4 判『数据量能否减』看 roofline.traffic_redundancy_read (实际读/理论最小, >1.5=重复搬运), "
            f"不只是看带宽利用率; task.transfers 的 bw 是估算 (每元素搬一次的下界), 只在无实测时用\n"
            ...
            f"## 融合分析 (Tier2 才有):\n{fusion_text}\n\n"
            f"## 任务:\n"
            f"0. ★每轮精准原则 (比什么都重要): 目标是**每一轮都调出可测效果**——"
            f"①只输出 1~2 个最有把握的 changes (改准优先于改多, 多改=高错误率); "
            f"②先在心里判断『瓶颈在哪个 kernel/哪个引擎』, 再对照速查卡选对应情况; "
            f"③拿不准就不改: 输出 promote (带 evidence) 或空 changes[]+说明, 绝不猜; "
            f"④上一轮 expected_impact vs 实际 speedup 的差距要反思 (历史里有)\n"
