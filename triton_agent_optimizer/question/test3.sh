...
echo "=== bisheng ==="
/usr/local/Ascend/cann-9.0.0/bin/bisheng -c k.asc -o k.o --npu-arch=dav-2201 --run-mode=sim 2>&1
echo "BISHENG EXIT: $?"
...
if [ -f app ]; then
  echo "=== ldd ==="
  ldd app 2>&1 | grep "not found" || echo "ALL LIBS OK"
  echo "=== run ==="
  ./app 2>&1; echo "RUN: $?"
  echo "=== msprof ==="
  ...
  echo "FILES: $(find prof -type f 2>/dev/null | wc -l)"
fi
