#!/usr/bin/env python3
"""PyTorch 转置 Matmul 基准线 — 与 input/matmul_transpose/kernel_op.py 同形状:
  C = A @ B^T  (A[M,K], B[N,K], M=N=K=2048, fp32)
  输出 pytorch_matmul_transpose_tflops.json
"""
...
try:
    ...
    pass
OUT = Path(__file__).resolve().parent / "outputs" / "pytorch_matmul_transpose_tflops.json"
...
def main():
    ...
    a = (torch.randn(args.m, args.k, dtype=dt, device=npu)) * 0.1
    b = (torch.randn(args.n, args.k, dtype=dt, device=npu)) * 0.1
    ...
    print(f"  torch matmul_transpose({args.m}x{args.n}x{args.k}, {args.dtype}): "
