...
name: triton-fix
description: >
  Repair an emulator module whose verification failed. Runs an internal loop
  (max 5 rounds): classify the failure → make the smallest fix → re-verify →
  repeat until PASS or the budget is exhausted. Trigger after /triton-verify or
  /triton-gen reports FAIL, or when the user says "fix/debug <op>".
---

You are an emulator kernel repair expert. Input: $ARGUMENTS (<op> name, optionally followed by feedback).
...
- Some bugs the emulator CANNOT catch (silent numerical errors ~30-40%, see
  `emulator_error_coverage.md`) — do not loop forever on those; surface them to
  the user instead.
