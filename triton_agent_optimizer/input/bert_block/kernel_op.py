...
# -*- coding: utf-8 -*-
# ═══════════════════════════════════════════════════════════════════════════════
#  单文件 kernel_op.py — BERT Encoder Block (post-LN) (v4.5)
#  ★ 优化循环里 coder 只改这一个文件; 读取/运行/测试都只看这一个文件
#  分三个区: ① 场景 config  ② 算子 kernel  ③ 测试 main
#
#  运算链 (BERT 原始 post-LN 结构):
#    A = (softmax((X@Wq)(X@Wk)^T·s) @ (X@Wv)) @ Wo
#    H1 = LN(X + A);  M = GELU(H1@W1+b1)@W2+b2;  Y = LN(M + H1)
#  算法: 与 input/vit_block 同套 MHA kernel (K_t/V_hsd 预转置, 窗口外一次性)
# ═══════════════════════════════════════════════════════════════════════════════
import os
import sys
import torch
...
# ═══════════════════════════════════════════════════════════════════════════════
S = int(os.environ.get("BERT_SEQ", 512))      # 序列长
D = int(os.environ.get("BERT_DIM", 768))      # 隐藏维
H = int(os.environ.get("BERT_HEADS", 12))     # 头数
HD = int(os.environ.get("BERT_HDIM", 64))     # 头维
FFN = int(os.environ.get("BERT_FFN", 3072))   # MLP 中间维
DTYPE = torch.float32
SCALE = 1.0 / (HD ** 0.5)
EPS = 1e-6
BLOCK_M, BLOCK_N, BLOCK_K = 64, 64, 64        # matmul 分块
BLOCK_S = 512                                 # softmax/LN 行分块 (≥S=512)
...
# ═══════════════════════════════════════════════════════════════════════════════
#  ② 算子 kernel (与 input/vit_block 同套)
# ═══════════════════════════════════════════════════════════════════════════════
...
@triton.jit
def add_kernel3(a_ptr, b_ptr, c_ptr, n, BLOCK: tl.constexpr):
    pid = tl.program_id(axis=0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < n
    a = tl.load(a_ptr + offs, mask=mask, other=0.0)
...
def main():
    ...
    a = torch.empty(S, D, dtype=DTYPE, device=npu)
    h1 = torch.empty(S, D, dtype=DTYPE, device=npu)
    ...
    print(f"[info] BERT block S={S} D={D} H={H} HD={HD} FFN={FFN} kernels=10")
    ...
    print("[info] BERT block launched & synced OK")
    ...
    if os.environ.get("MATMUL_VERIFY", "0") == "1":
        ...
        a_r = (ph @ vh).transpose(0, 1).reshape(S, HD_D) @ wo
        h1_r = F.layer_norm(x + a_r, [D], wln, bln, EPS)
        m_r = F.gelu(h1_r @ w1 + b1) @ w2 + b2
        ref = F.layer_norm(m_r + h1_r, [D], wln, bln, EPS)
