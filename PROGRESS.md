...
> **env var 统一 AGENT_*（去硬编码 nga）**：CLI backend 环境变量统一 `AGENT_*` 前缀——`NGA_GENERATE_MODEL`/`NGA_CHOOSE_LEVER_MODEL`→`AGENT_GENERATE_MODEL`/`AGENT_CHOOSE_LEVER_MODEL`；默认命令前缀 `["nga","run"]`→`["agent","run"]`。`preflight.py`：`_check_llm` 读 `AGENT_*_MODEL`、`_check_nga`→`_check_agent`（按 `AGENT_CMD` 首 token 探测二进制，缺省 `agent`，状态键 `nga`→`agent`）。测试/文档同步（t13_7、v2_backend_unified、e6、V2_ENV_ADAPTATION §A、V2_IMPLEMENTATION_GUIDE）。**注**：按用户明确指示覆盖 GLM52 期冻结的 `NGA_*` 环境变量名（属环境侧配置、非框架契约字段）；类名 `NgaBackend` 保留（标识符、非环境变量）。`pytest -q` = **239 passed**。
