...
# -*- coding: utf-8 -*-
# ═══════════════════════════════════════════════════════════════════════════════
#  单文件 kernel_op.py — Vector Add (纯逐元素) (v4)
#  ★ 优化循环里 coder 只改这一个文件; 读取/运行/测试都只看这一个文件
#  分三个区: ① 场景 config  ② 算子 kernel  ③ 测试 main
#
#  运算: Y = X + Z   (逐元素, 1D)
#  单 kernel, 纯逐元素 (无归约/无 matmul) — Tier4/5 优化目标 (访存/向量指令)
#
# ═══════════════════════════════════════════════════════════════════════════════
import os
import sys
...
# ═══════════════════════════════════════════════════════════════════════════════
#  ① 场景 config — 尺寸/精度/分块
#  形状: X[N], Z[N] → Y[N] (1D 扁平)
# ═══════════════════════════════════════════════════════════════════════════════
N = int(os.environ.get("VEC_N", 4 * 1024 * 1024))   # 元素数 (4M, 够大才有带宽压力)
DTYPE = torch.float32
...
# Vector Add: 逐元素. 3 指针 (2 load + 1 store), 1 加法.
@triton.jit
def add_kernel(x_ptr, z_ptr, y_ptr,
               n_elements,
               BLOCK: tl.constexpr):
    ...
    tl.store(y_ptr + offs, x + z, mask=mask)
...
def main():
    ...
    print(f"[info] vector_add N={N} dtype={DTYPE} grid={grid[0]} block={BLOCK_SIZE}")

    # ★KERNEL_LOOP: verify/bench 用它 (一次 msprof 内循环 N 次, 求单次平均; 分配只做一次复用)
    LOOP = int(os.environ.get("KERNEL_LOOP", "1"))
    for _ in range(LOOP):
        add_kernel[grid](x, z, y, N, BLOCK=BLOCK_SIZE)
    torch.npu.synchronize()
    print("[info] vector_add launched & synced OK")
