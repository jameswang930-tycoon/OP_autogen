...
# -*- coding: utf-8 -*-
# ═══════════════════════════════════════════════════════════════════════════════
#  单文件 kernel_op.py — Fused Add+Mul (纯逐元素) (v4)
#  ★ 优化循环里 coder 只改这一个文件; 读取/运行/测试都只看这一个文件
#  分三个区: ① 场景 config  ② 算子 kernel  ③ 测试 main
#
#  运算: Y = (X + Z) * W   (逐元素, 1D; 3 输入融合)
#  单 kernel, 纯逐元素 (无归约/无 matmul) — Tier2 融合(3 load→1 store) 优化示例
#
# ═══════════════════════════════════════════════════════════════════════════════
import os
import sys
...
# ═══════════════════════════════════════════════════════════════════════════════
#  ① 场景 config — 尺寸/精度/分块
#  形状: X[N], Z[N], W[N] → Y[N] (1D 扁平)
# ═══════════════════════════════════════════════════════════════════════════════
N = int(os.environ.get("FAM_N", 4 * 1024 * 1024))   # 元素数 (4M, 够大才有带宽压力)
DTYPE = torch.float32
...
# Fused Add+Mul: 逐元素. 4 指针 (3 load + 1 store), 1 add + 1 mul.
@triton.jit
def fused_add_mul_kernel(x_ptr, z_ptr, w_ptr, y_ptr,
                         n_elements,
                         BLOCK: tl.constexpr):
    ...
    tl.store(y_ptr + offs, (x + z) * w, mask=mask)
...
def main():
    ...
    z = (torch.randn(N, dtype=DTYPE, device=npu)) * 0.1
    w = (torch.randn(N, dtype=DTYPE, device=npu)) * 0.1
    y = torch.empty(N, dtype=DTYPE, device=npu)

    grid = (triton.cdiv(N, BLOCK_SIZE),)
    print(f"[info] fused_add_mul N={N} dtype={DTYPE} grid={grid[0]} block={BLOCK_SIZE}")

    # ★KERNEL_LOOP: verify/bench 用它 (一次 msprof 内循环 N 次, 求单次平均; 分配只做一次复用)
    LOOP = int(os.environ.get("KERNEL_LOOP", "1"))
    for _ in range(LOOP):
        fused_add_mul_kernel[grid](x, z, w, y, N, BLOCK=BLOCK_SIZE)
    torch.npu.synchronize()
    print("[info] fused_add_mul launched & synced OK")
