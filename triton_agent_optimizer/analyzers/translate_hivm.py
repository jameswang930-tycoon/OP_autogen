...
OP_SEM = {
    ...
    "mmad": ("CubeUnit", "矩阵乘加 MMAD (通用 Cube)"),
    "batch": ("CubeUnit", "批量操作标记 batch"),
    "mix_matmul": ("CubeUnit", "矩阵乘法 + 后向量函数 tile 级融合 (CV 混合)"),
    "mmadL1": ("CubeUnit", "矩阵乘加 C=C+A×B(+bias): L1 → L0C (Cube 核心, 我们的主计算, PIPE_MTE1+PIPE_M)"),
    ...
    "sync_block": ("Sync", "同步-块同步 (跨 kernel 栅栏; 模式: ALL_CUBE/ALL_VECTOR/BARRIER_CUBE/ALL)"),
    ...
    # ── 其余向量运算 (文档: PIPE_V 向量核, VectorCoreTypeTrait) ──
    "vand": ("VecUnit", "向量按位与 AND"),
    "varange": ("VecUnit", "向量生成等差数列 range"),
    "vcast": ("VecUnit", "向量类型转换 cast"),
    "vconcat": ("VecUnit", "向量拼接 concat"),
    "vcos": ("VecUnit", "向量余弦 cos"),
    "vcumprod": ("VecUnit", "向量累积乘积 cumprod"),
    "vcumsum": ("VecUnit", "向量累积和 cumsum"),
    "vdeinterleave": ("VecUnit", "向量解交织 deinterleave"),
    "verf": ("VecUnit", "向量误差函数 erf"),
    "vflip": ("VecUnit", "向量翻转 flip"),
    "vgather": ("VecUnit", "向量聚集 gather (按索引取元素)"),
    "vinterleave": ("VecUnit", "向量交织 interleave"),
    "vmod": ("VecUnit", "向量取模 mod"),
    "vmulext": ("VecUnit", "向量扩展乘法 mulext"),
    "vmulextended": ("VecUnit", "向量扩展乘法 (extended)"),
    "vnot": ("VecUnit", "向量按位非 NOT"),
    "vor": ("VecUnit", "向量按位或 OR"),
    "vpad": ("VecUnit", "向量填充 pad"),
    "vpow": ("VecUnit", "向量幂运算 pow"),
    "vrec": ("VecUnit", "向量倒数 reciprocal"),
    "vreduce": ("VecUnit", "向量归约 reduce (求和/求最值等)"),
    "vshl": ("VecUnit", "向量左移 shl"),
    "vshr": ("VecUnit", "向量右移 shr"),
    "vsin": ("VecUnit", "向量正弦 sin"),
    "vsort": ("VecUnit", "向量排序 sort"),
    "vtranspose": ("VecUnit", "向量转置 transpose"),
    "vxor": ("VecUnit", "向量异或 XOR"),
    # ── 布局转换 / 拷贝 / 杂项 ──
    "nd2nz": ("VecUnit", "布局转换 ND→NZ (fractal, Cube 输入格式)"),
    "nz2nd": ("VecUnit", "布局转换 NZ→ND (还原 ND)"),
    "convert_layout": ("VecUnit", "布局转换 convert_layout"),
    "bitcast": ("VecUnit", "位重解释 bitcast"),
    "copy": ("VecUnit", "数据拷贝 copy"),
    "pointer_cast": ("Scalar", "指针类型转换 pointer_cast"),
    "dcci": ("Scalar", "DCCI (ICACHE 相关指令)"),
    "set_mask_norm": ("Scalar", "设置掩码归一化 set_mask_norm"),
    "set_ffts_base_addr": ("Scalar", "设置 FFTS 基地址 (910B 同步用)"),
    "custom": ("?", "自定义操作 custom"),
    # ── 原子操作 ──
    "atomic_rmw": ("Atomic", "原子读改写 read-modify-write"),
    "atomic_xchg": ("Atomic", "原子交换 xchg"),
    # ── 块同步 (SyncBlock 系列, 跨 kernel) ──
    "create_sync_block_lock": ("Sync", "创建块同步锁"),
    "sync_block_lock": ("Sync", "块同步-加锁 lock_var"),
    "sync_block_unlock": ("Sync", "块同步-解锁 lock_var"),
    "sync_block_set": ("Sync", "块同步-置位 [tcore_type, tpipe, pipe]"),
    "sync_block_wait": ("Sync", "块同步-等待 [tcore_type, tpipe, pipe]"),
    # ── 调试 ──
    "debug": ("Debug", "调试指令"),
    "init_debug": ("Debug", "调试初始化"),
    "finish_debug": ("Debug", "调试结束"),
}
